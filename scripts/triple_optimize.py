...
def jacobian(x):
    ...
    fderivs2 = read_solution_file("fderivs_0002-temp.bin")
    Jf0 = 2.0/n*((current_v-interped_v)*fderivs0[var]*f_ref).sum()
    Jf1 = 2.0/n*((current_v-interped_v)*fderivs1[var]*f_ref).sum()
    Jf2 = 2.0/n*((current_v-interped_v)*fderivs2[var]*f_ref).sum()

    Hderivs = read_solution_file("Hderivs-temp.bin")
    Hderivs_v = Hderivs[var]
    JH = 2.0/n*((current_v-interped_v)*Hderivs_v*H_ref).sum()

    print("       jac with f=[{},{},{}] H={} and output={},{},{},{}".format(f0/f_ref,f1/f_ref,f2/f_ref,H/H_ref,Jf0,Jf1,Jf2,JH))
    return array([Jf0, Jf1, Jf2, JH])

#var = 'v'
var = 'M'
target = read_solution_file('triple_combined.bin')
...
H = 7.0
x0 = array([f0, f1, f2, H])
print("Starting with f=[{:5.5f},{:5.5f},{:5.5f}] H={:5.5e}".format(f0, f1, f2, H))
...
res = minimize(objective, x0, bounds=(array([0.0, 100.0]),array([0.0, 100.0]),array([0.0, 100.0]),array([0.0, 20.0])), method='SLSQP', jac=jacobian, tol=1e-8, options={'disp':True})

print("Done... found f0,f1,f2,H={})".format(res.x))
print(res)
